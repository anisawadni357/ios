...
@Component({
  selector: 'app-liste-following-user',
  templateUrl: './liste-following-user.page.html',
  styleUrls: ['./liste-following-user.page.scss'],
})
export class ListeFollowingUserPage implements OnInit {
  baseUrlFile=environment.BASE_URL_file;
  constructor(private profileService:ProfileService,private route: ActivatedRoute,private alertController: AlertController) { }
  liste : any=[];
  id:any=0;
  userId:any=0;
  ngOnInit() {
  ...
  getData()
  {
    this.profileService.getAbonne(this.id)
    .subscribe(
      data => {
