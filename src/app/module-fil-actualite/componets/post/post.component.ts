...
async showSuccessToast(msg:any) {
  ...
  toast.present(); 
    ...
    partagerVideoEtTexte() {
    
      const message = this.post.text;

      var videoUrl="";
      if(this.file[0])videoUrl = this.baseUrlFile+this.file[0];
          // Remplace par l'URL ou le chemin local de la vidéo
      const url = "https://example.com"; // Lien associé (facultatif)
  
      this.socialSharing.share(message, "", videoUrl)
        .then(() => {
          console.log("Contenu partagée avec succès !");
        })
        .catch((error:any) => {
        ...
        buttons: [
          {
            text: 'Share',
            icon: 'share',
            cssClass: 'share-class',  // Classe CSS pour Share
            data: 10,
            handler: () => {
          ...
          },
          {
            text: 'Signal',
            icon: 'warning',
            cssClass: 'signal-class',  // Classe CSS pour Signal
            role: 'single',
            handler: () => {
             this.signale();
            }
